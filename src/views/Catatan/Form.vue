			...
			<i-row>
				<i-column offset-md="1" md="5">
					...
					<v-select
						...
						label="id"
						:filterable="false"
						:options="Siswa.Search"
						:loading="Siswa.Loading"
						v-model="FormMain.siswa"
				...
				<i-column md="5" class="_margin-bottom-1">
					<FormulateInput
						...
						name="peristiwa"
						label="Peristiwa"
						placeholder="Penjelasan Peristiwa .."
						rows="4"
					/>
					...
					<FormulateInput
						...
						name="evaluasi"
						label="Evaluasi"
						placeholder="Evaluasi untuk guru atau orangtua .."
					/>
					...
					<FormulateInput
						...
						name="keterangan"
						label="Keterangan"
						placeholder="Keterangan cara menangani peristiwa .."
					/>
				</i-column>
			</i-row>
		</FormulateForm>
...
export default {
	...
	mixins: [MixinCreate],
	props: {
		ModelName: { type: String, default: "Catatan" },
	},
	...
	methods: {
		...
		SaveData() {
			...
			this.FormMain.fk_siswa = this.FormMain.siswa.id;
			this.$store.dispatch(`${this.ModelName}/PostForm`).then((response) => {
				if (response.status < 400) {
					...
					this.ExitForm();
						...
						if (!isEmptyObject(response.data)) this.FormErrors.Field = response.data;
		...
		onSearchSiswa: debounce(function (search, loading) {
			...
			Search("siswa/search", {
				value: escape(search),
				column: "nama",
			}, {}, {_with:['kelas']}).then((res) => {
				if (res.status < 400) this.Siswa.Search = res.data.data.filter(item => !isEmptyObject(item.kelas));
				loading(false);
			});
		}, 350),
