<template>
	...
	>
		...
		>
			<i-row bottom-md>
				<i-column offset-md="1" md="10">
					<div class="blue-cloud _rounded" style="height: 80px;">
						<img
		...
		<i-row v-if="!IsCreate">
			<i-column offset-md="1" md="10">
				<h5 class="_margin-top-1_text-gray-70">List Indikator</h5>
			</i-column>
			<i-column  md="4" offset-xs="1" xs="10">
				<i-input
					class="_margin-bottom-1"
					v-model="Indikator.Search"
					size="sm"
					placeholder="Cari data ..."
					v-on:input="OnIndikatorCari"
				>
			...
			<i-column offset-md="1" md="10">
				<div>
					<Table
						:Simple="true"
						:NoHead="true"
						v-on:page-change="LoadIndikator"
						v-on:sort-change="OnIndikatorSort"
						:Rows="Indikator.Data"
						:Columns="Indikator.Columns"
						:Meta="Indikator.Meta"
...
import { UPFile, GETTable } from "../../utils/net";
...
export default {
	name: "Pembelajaran-Create",
	mixins: [MixinCreate],
	props: {
		ModelName: { type: String, default: "Pembelajaran" },
	},
	components: { Table, SimpleForm, Fa },
	data() {
		return {
			faSearch,
			faTimesCircle,
	...
	methods: {
		...
		OnIndikatorSort(column, index) {
			this.$set(this.Indikator.Columns, index, column);
			let orders = [];
			this.Indikator.Columns.forEach((column) => {
		...
		LoadIndikator(page = 1) {
			this.Indikator.Loading = true;
			this.Indikator.Meta.page = page;

			let meta = this.Indikator.Meta;
			GETTable(`pembelajaran/${this.id}/indikator`, meta).then((response) => {
				this.Indikator.Meta = {
					...this.Indikator.Meta,
					...(response.data.meta ?? {}),
				};
				this.Indikator.Loading = false;
				if (response.status < 400) this.Indikator.Data = response.data.data;
				else this.Indikator.Data = [];
			});
		},
	...
	async mounted() {
		this.OnIndikatorCari = debounce(() => {
			this.Indikator.Meta = {
				...this.Indikator.Meta,
				_like: {
					_text: this.Indikator.Search,
					_columns: ["nama"],
				},
			};
			this.LoadIndikator();
