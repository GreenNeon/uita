...
export default {
	actions: {
		SetItems (context) {
			...
			GETTable('siswa', context.state.Meta).then((response) => {
		...
		async SetItem (context, id) {
			context.commit('SET_LOADING', true)
			return await GETData(`siswa/${id}`).then((response) => {
		...
		async PostForm (context) {
			...
			return POSTData('siswa', context.state.Form).then((response) => {
				context.commit('SET_LOADING', false)
				return response
			})
		},
		async PatchForm (context, id) {
			...
			return PATCHData(`siswa/${id}`, context.state.Form).then((response) => {
				context.commit('SET_LOADING', false)
		...
		async DeleteItem (context, id = -1) {
			...
			return DELData(`siswa/${id}`, data).then(response => {
				context.commit('SET_LOADING', false)
