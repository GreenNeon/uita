...
export default {
	actions: {
		SetItems (context) {
			...
			GETTable('indikator', context.state.Meta).then((response) => {
		...
		async SetItem (context, id) {
			context.commit('SET_LOADING', true)
			return GETData(`indikator/${id}`).then((response) => {
		...
		async PostForm (context) {
			...
			return POSTData('indikator', context.state.Form).then((response) => {
				context.commit('SET_LOADING', false)
		...
		async PatchForm (context, id) {
			...
			return PATCHData(`indikator/${id}`, context.state.Form).then((response) => {
				context.commit('SET_LOADING', false)
				if (response.status < 400) context.commit('CLEAR_FORM')
				return response
			})
		...
		async DeleteItem (context, id) {
			context.commit('SET_LOADING', true)
			return DELData(`indikator/${id}`).then(response => {
				context.commit('SET_LOADING', false)
