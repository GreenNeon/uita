...
export default {
	actions: {
		SetItems (context) {
			...
			GETTable('pembelajaran', context.state.Meta).then((response) => {
		...
		async SetItem (context, id) {
			context.commit('SET_LOADING', true)
			return GETData(`pembelajaran/${id}`).then((response) => {
		...
		async PostForm (context) {
			...
			return POSTData('pembelajaran', context.state.Form).then((response) => {
				context.commit('SET_LOADING', false)
		...
		async PatchForm (context, id) {
			...
			return PATCHData(`pembelajaran/${id}`, context.state.Form).then((response) => {
				context.commit('SET_LOADING', false)
				if (response.status < 400) context.commit('CLEAR_FORM')
				return response
			})
		...
		async DeleteItem (context, id) {
			context.commit('SET_LOADING', true)
			return DELData(`pembelajaran/${id}`).then(response => {
				context.commit('SET_LOADING', false)
