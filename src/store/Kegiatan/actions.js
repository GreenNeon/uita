...
export default {
	actions: {
		SetItems (context) {
			...
			GETTable('kegiatan', context.state.Meta).then((response) => {
		...
		async SetItem (context, id) {
			context.commit('SET_LOADING', true)
			return GETData(`kegiatan/${id}`).then((response) => {
		...
		async PostForm (context) {
			...
			return POSTData('kegiatan', context.state.Form).then((response) => {
				context.commit('SET_LOADING', false)
		...
		async PatchForm (context, id) {
			...
			return PATCHData(`kegiatan/${id}`, context.state.Form).then((response) => {
				context.commit('SET_LOADING', false)
				if (response.status < 400) context.commit('CLEAR_FORM')
				return response
			})
		...
		async DeleteItem (context, id) {
			context.commit('SET_LOADING', true)
			return DELData(`kegiatan/${id}`).then(response => {
				context.commit('SET_LOADING', false)
