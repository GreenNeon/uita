...
export default {
	actions: {
		SetItems (context) {
			...
			GETTable('kelas', context.state.Meta).then((response) => {
		...
		async SetItem (context, id) {
			context.commit('SET_LOADING', true)
			return GETData(`kelas/${id}`).then((response) => {
		...
		async PostForm (context) {
			...
			return POSTData('kelas', context.state.Form).then((response) => {
				context.commit('SET_LOADING', false)
		...
		async PatchForm (context, id) {
			...
			return PATCHData(`kelas/${id}`, context.state.Form).then((response) => {
				context.commit('SET_LOADING', false)
				if (response.status < 400) context.commit('CLEAR_FORM')
				return response
			})
		...
		async DeleteItem (context, id) {
			context.commit('SET_LOADING', true)
			return DELData(`kelas/${id}`).then(response => {
				context.commit('SET_LOADING', false)
		...
		GET_CLEAN_FORM (context) {
			...
			delete forms[ 'kelompok' ]
			delete forms[ 'fk_foto' ]
			delete forms[ 'foto' ]
			return (forms)
		}
	},
