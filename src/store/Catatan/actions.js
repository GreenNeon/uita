...
export default {
	actions: {
		SetItems (context) {
			...
			GETTable('catatan', context.state.Meta).then((response) => {
		...
		async SetItem (context, id) {
			context.commit('SET_LOADING', true)
			return GETData(`catatan/${id}`).then((response) => {
		...
		async PostForm (context) {
			...
			return POSTData('catatan', context.state.Form).then((response) => {
				context.commit('SET_LOADING', false)
		...
		async PatchForm (context, id) {
			...
			return PATCHData(`catatan/${id}`, context.state.Form).then((response) => {
				context.commit('SET_LOADING', false)
				if (response.status < 400) context.commit('CLEAR_FORM')
				return response
			})
		...
		async DeleteItem (context, id) {
			context.commit('SET_LOADING', true)
			return DELData(`catatan/${id}`).then(response => {
				context.commit('SET_LOADING', false)
		...
		GET_CLEAN_FORM (context) {
			...
			delete forms[ 'foto' ]
			delete forms[ 'tanggal' ]
			delete forms[ 'fk_siswa' ]
			return (forms)
		}
	},
	getters: {
