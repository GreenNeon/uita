...
export default {
	actions: {
		SetItems (context) {
			...
			GETTable('orangtua', context.state.Meta).then((response) => {
		...
		async SetItem (context, id) {
			context.commit('SET_LOADING', true)
			return GETData(`orangtua/${id}`).then((response) => {
		...
		async PostForm (context) {
			...
			return POSTData('orangtua', context.state.Form).then((response) => {
				context.commit('SET_LOADING', false)
		...
		async PatchForm (context, id) {
			...
			return PATCHData(`orangtua/${id}`, context.state.Form).then((response) => {
				context.commit('SET_LOADING', false)
				if (response.status < 400) context.commit('CLEAR_FORM')
				return response
			})
		...
		async DeleteItem (context, id = -1) {
			...
			return DELData(`orangtua/${id}`, data).then(response => {
				context.commit('SET_LOADING', false)
